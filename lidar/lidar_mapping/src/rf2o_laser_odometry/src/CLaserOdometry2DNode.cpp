...
{
    RCLCPP_WARN(get_logger(), ">>> LaserCallBack called! GT_init=%d first_scan=%d", GT_pose_initialized, rf2o_ref.first_laser_scan);
...
void CLaserOdometry2DNode::publish()
{
  ...
  if (publish_tf)
  {
    ...
    rclcpp::Time future_stamp = this->now() + rclcpp::Duration(0, 300000000);
    odom_trans.header.stamp = future_stamp;    // the time of the last scan used!
