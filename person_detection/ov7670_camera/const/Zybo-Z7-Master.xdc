...
create_clock -period 40.000 -name ov_pclk_clk -waveform {0.000 20.000} [get_ports ov_pclk]
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets ov_pclk_IBUF]

set_false_path -from [get_clocks sys_clk_pin] -to [get_clocks ov_pclk_clk]
set_false_path -from [get_clocks ov_pclk_clk] -to [get_clocks sys_clk_pin]
...
##LEDs
set_property -dict { PACKAGE_PIN M14   IOSTANDARD LVCMOS33 } [get_ports init_done];   # LED[0]: 초기화 완료
set_property -dict { PACKAGE_PIN M15   IOSTANDARD LVCMOS33 } [get_ports capturing];   # LED[1]: 캡처 중
set_property -dict { PACKAGE_PIN G14   IOSTANDARD LVCMOS33 } [get_ports frame_done];  # LED[2]: 프레임 완료 (깜빡임)
set_property -dict { PACKAGE_PIN D18   IOSTANDARD LVCMOS33 } [get_ports frame_valid]; # LED[3]: 데이터 유효 (깜빡임)
...
set_property -dict { PACKAGE_PIN Y7    IOSTANDARD LVCMOS33     } [get_ports ov_sda];   # JB7: SCCB Data
set_property -dict { PACKAGE_PIN Y6    IOSTANDARD LVCMOS33     } [get_ports ov_scl];   # JB8: SCCB Clock
